...
# For y, just predict the number of cases
# Use a linear regression later to predict number of deaths
...
with graph.as_default():
    ...
    with tf.name_scope("fcn5"):
        ...
        predictions = tf.nn.elu(predictions) + 1
    # loc is mean, scale is sd
    dist = tfp.distributions.Normal(loc=predictions[:, 0], scale=predictions[:, 1])
    loss = tf.reduce_mean(-dist.log_prob(labels))
